...
//   const products: Product[] = useMemo(() => productsResponse?.result || [], [productsResponse]);
//   const uniqueCategories: Category[] = useMemo(() => categoriesResponse?.content || [], [categoriesResponse]);


//   const uniqueUnits = useMemo(() => {
//     const units = new Set<string>();
//     products.forEach(p => p.unit && units.add(p.unit)); 
...
//         (product.productId?.toString().toLowerCase().includes(lowerCaseSearchTerm)) || 
//         (product.name?.toLowerCase().includes(lowerCaseSearchTerm))
//       );
//     }

//     if (filterCategory !== 'all') {
//       tempProducts = tempProducts.filter(product => product.category.name === filterCategory);
//     }

//     return tempProducts;
//   }, [products, searchTerm, filterStockMin, filterStockMax, filterUnit, filterStatus, filterCategory]);

//   const handleToggleStatus = (productId: string | number) => { /* ... giữ nguyên ... */ };
//   const handleEditProduct = (product: Product) => { /* ... giữ nguyên ... */ };
//   const confirmDeleteProduct = () => { /* ... giữ nguyên ... */ };
...
//         <p className="text-gray-500 mt-2">Không thể tải danh sách sản phẩm. Vui lòng thử lại sau.</p>
//       </div>
//     );
//   }
...
//           {/* Wang Code Web: Lấy danh sách danh mục từ API */}
